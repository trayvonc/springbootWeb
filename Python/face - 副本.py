...
def recognize_face(queueMsg):
    ...
    for item in ps.listen():
        if item['type'] == 'message':
            ...
            redisCli.publish("channel:1", str(result_status) + "_" + result_msg)
            print("结果:", str(result_status) + "_" + result_msg)



def upload_faceid(queueMsg):
    print("上传人脸")
    redisCli = redis.Redis(host='127.0.0.1', port=6379)
    ps = redisCli.pubsub()
    for item in ps.listen():
...
if __name__ == '__main__':
    ...
    # ps.subscribe(**{"checkFace2": recognize_face})
    # ps.subscribe(**{"checkFace3": recognize_face})
    # ps.subscribe(**{"checkFace4": recognize_face})
    # ps.subscribe(**{"checkFace0": recognize_face})
    # ps.subscribe(**{"uploadFace1": upload_faceid})
    # ps.subscribe(**{"uploadFace2": upload_faceid})
    # ps.subscribe(**{"uploadFace3": upload_faceid})
    # ps.subscribe(**{"uploadFace4": upload_faceid})
    # ps.subscribe(**{"uploadFace0": upload_faceid})
